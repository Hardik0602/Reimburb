...
import { RiArrowRightLine, RiCheckLine, RiCloseLine, RiDashboardLine, RiFileListLine, RiMenuLine, RiTeamLine, RiTimeLine } from "react-icons/ri"
...
const nav: NavGroup[] = [
  {
    group: 'Overview',
    item: [
      { label: 'Dashboard', to: '/manager', icon: <RiDashboardLine /> },
      { label: 'My Team', to: '/manager/team', icon: <RiTeamLine /> },
      { label: 'Team Expenses', to: '/manager/expenses', icon: <RiFileListLine /> }
    ]
  },
  {
    group: 'Approvals',
    item: [
      { label: 'Pending', to: '/manager/pending', icon: <RiTimeLine />, badge: 2, badgeStyle: 'bg-amber-500/10 text-amber-400' },
      { label: 'Approved', to: '/manager/approved', icon: <RiCheckLine /> },
      { label: 'Rejected', to: '/manager/rejected', icon: <RiCloseLine /> }
    ]
  }
]
const ManagerNavBar = () => {
  const location = useLocation()
  const isActive = (to: string) => to === '/manager' ? location.pathname === '/manager' : location.pathname.startsWith(to)
    ...
    <>
          ...
          <div
            ...
            {group.item.map((i) => (
              <Link
                ...
                to={i.to}
                onClick={onPress}
                className={`flex items-center gap-2.5 px-2.5 py-2 rounded-lg text-[13px] font-semibold mb-0.5 transition-all ${isActive(i.to) ? 'bg-[#2eddbb]/10 text-[#2eddbb]' : 'text-white/50 hover:bg-white/5 hover:text-white/80'}`}>
      ...
      <div className="border-t border-white/6 px-4 py-3.5 flex items-center justify-between">
        <div className="flex gap-2.5">
          <div className="w-8.5 h-8.5 rounded-[9px] bg-linear-to-br from-[#0F6E56] to-[#2eddbb] flex items-center justify-center text-[12px] font-bold text-white">
            TH
          </div>
